...
CLI today: ``python -m alpha_engine_lib.contracts validate <slot> <path.json>``.
...
Requires the ``contracts`` extra (``alpha-engine-lib[contracts]`` → jsonschema).
"""
...
def _validator(name: str):
    ...
    try:
        ...
        raise ImportError(
            "alpha_engine_lib.contracts needs the 'contracts' extra: "
            "pip install 'alpha-engine-lib[contracts]'"
        ) from exc
